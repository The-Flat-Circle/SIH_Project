...
export default function HeroSection({ densityLevel, setDensityLevel }: HeroSectionProps) {
  ...
  return (
    <section id="overview" className="relative min-h-screen pt-32 pb-20 overflow-hidden">
      {/* Background Image: Authentic Shree Jagannath Puri Temple with Rich Glass & Shadow Overlay */}
      <div className="absolute inset-0 z-0">
        <Image
          ...
          fill
          priority
          className="object-cover object-center scale-105 filter brightness-[0.45] contrast-[1.1] blur-[1px]"
        />
        {/* Layered Gradient Overlays for High-Contrast Readability & Temple Gold Radiance */}
        <div className="absolute inset-0 bg-gradient-to-b from-stone-charcoal/95 via-stone-charcoal/80 to-stone-charcoal z-1" />
        <div className="absolute inset-0 bg-radial-gradient from-temple-gold/15 via-transparent to-stone-charcoal/90 z-2" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-dusk-indigo/40 via-stone-charcoal/70 to-stone-charcoal z-2" />
      </div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {/* Top Operational Tag */}
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-temple-gold/40 bg-stone-charcoal/80 text-xs font-mono text-parchment mb-8 backdrop-blur-md shadow-[0_0_20px_rgba(201,151,62,0.2)]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          {/* Left Column: Copy & Interactive Simulator Card */}
          <div className="lg:col-span-6 space-y-6">
            <h1 className="font-serif text-4xl sm:text-5xl lg:text-6xl font-normal leading-[1.12] text-parchment tracking-tight drop-shadow-md">
            ...
            <p className="text-base sm:text-lg text-sandstone/90 font-normal leading-relaxed max-w-xl">
            ...
            <div className="space-y-2 pt-1">
              <span className="text-[11px] font-mono tracking-widest text-sandstone/80 uppercase flex items-center gap-1.5">
              ...
              <div className="flex flex-wrap gap-2">
                {heritageSites.map((site) => (
                  <span
                    key={site.name}
                    className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-mono border backdrop-blur-md transition-all ${
                      site.active
                        ? "bg-dusk-indigo/90 text-temple-gold border-temple-gold/60 font-semibold shadow-[0_0_12px_rgba(201,151,62,0.25)]"
                        : "bg-stone-dark/70 text-sandstone/80 border-sandstone/20"
                    }`}
                  >
                    {site.name}
            ...
            <div className="p-5 rounded-2xl border border-sandstone/30 bg-stone-charcoal/80 backdrop-blur-xl space-y-3.5 shadow-2xl">
            ...
            <div className="flex flex-wrap items-center gap-4 pt-2">
              <a
                href="#dual-flow"
                className="inline-flex items-center gap-2 px-6 py-3.5 rounded-full text-xs font-mono font-bold uppercase tracking-wider text-stone-charcoal bg-gradient-to-r from-temple-gold via-temple-amber to-temple-gold hover:scale-105 border border-temple-gold/60 transition-all shadow-[0_0_25px_rgba(201,151,62,0.4)] active:scale-95"
              >
              ...
              <a
                href="#pipeline"
                className="inline-flex items-center gap-2 px-6 py-3.5 rounded-full text-xs font-mono tracking-wider text-parchment border border-sandstone/30 hover:border-temple-gold/60 bg-stone-charcoal/60 backdrop-blur-md hover:bg-stone-charcoal/80 transition-all"
              >
                System Architecture
              </a>
            </div>
