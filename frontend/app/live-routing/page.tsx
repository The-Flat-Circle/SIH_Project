...
import { ArrowLeft, Upload, FileVideo, Cpu, Play, Pause, CheckCircle2, ShieldAlert, ArrowRight, Compass, Ticket, QrCode, RefreshCw, Eye, BarChart2 } from "lucide-react";
...
export default function LiveRoutingPage() {
  ...
  // Pre-configured YOLOv8 Model Data (Generated by computer_vision/CV/main.py)
  const sampleData = {
    sample1: {
      name: "Sample 1: Singhadwara Gate Surge (YOLOv8 Real Crowd Output)",
    ...
    sample2: {
      name: "Sample 2: Night Queue Corridor (ByteTrack Scaled Output)",
  ...
  // Run YOLOv8 Execution Progress Simulation for Custom File
  const runYoloAnalysis = (filename: string) => {
    ...
    const stages = [
      "Loading PyTorch YOLOv8 Neural Weights (yolov8n.pt)...",
      "Parsing Video Resolution & Scaling Geometry...",
      "Executing YOLOv8 Person Class Detections (Conf > 0.35)...",
      "Running ByteTrack Multi-Object ID Assignment...",
      "Calculating Flow Speeds & Virtual Gate Line Crossings...",
      "Generating Structured Handoff Payload JSON...",
      "YOLOv8 Model Inference Complete! Rendering Output Feed...",
    ];
  ...
  const handleSelectSample = (key: "sample1" | "sample2") => {
    ...
    setVideoFile(null);
    setVideoSrc(sampleData[key].src);
    runYoloAnalysis(sampleData[key].name);
  };
  ...
  return (
    <div className="min-h-screen bg-stone-charcoal text-parchment font-sans relative selection:bg-temple-gold selection:text-stone-charcoal">
      ...
      <main className="pt-28 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-sandstone/15 pb-6">
          <div>
            ...
            <h1 className="font-serif text-3xl sm:text-4xl lg:text-5xl font-normal text-parchment">
              YOLOv8 Video Testing & <span className="italic text-temple-gold">Rerouting Engine</span>
            </h1>
            <p className="text-sandstone text-sm max-w-2xl mt-1">
              Upload any test video or select a sample to run our real-time YOLOv8 + ByteTrack model, inspect person tracking overlays, and generate live gate redistribution recommendations.
            </p>
          </div>
        </div>
            ...
            >
              ...
              <p className="text-sandstone text-xs max-w-sm mt-1">
                {videoFile
                  ...
                  : "Upload CCTV recordings or test videos to execute our Python YOLOv8 person tracking pipeline."}
              </p>
            ...
            <div className="lg:col-span-5 space-y-3">
              ...
              >
                <div>
                  ...
                  <div className="text-[11px] font-mono text-sandstone">Daytime Overcrowding • YOLOv8 Model Processed</div>
                </div>
              ...
              >
                <div>
                  ...
                  <div className="text-[11px] font-mono text-sandstone">Night Stream • ByteTrack Scaled Output</div>
                </div>
                <Play className="w-4 h-4 text-temple-gold" />
              </div>
            </div>
          ...
          <section className="p-6 rounded-2xl bg-dusk-card border border-temple-gold/40 space-y-4 shadow-2xl animate-pulse">
            <div className="flex items-center justify-between text-xs font-mono">
              <span className="text-amber-400 font-bold flex items-center gap-2">
                ...
                RUNNING YOLOV8 + BYTETRACK MODEL INFERENCE...
              </span>
        ...
        {/* SECTION 2: CLEAN VIDEO PLAYER WITH REAL YOLOV8 MODEL OVERLAYS */}
        {analysisDone && (
          <section className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="font-serif text-2xl text-parchment font-semibold flex items-center gap-2">
                <Eye className="w-5 h-5 text-emerald-400" />
                2. YOLOv8 Model Processed Video Feed & Flow Analysis
              </h2>
            ...
            <div className="relative rounded-2xl overflow-hidden bg-stone-dark border border-sandstone/30 shadow-2xl h-[380px] sm:h-[480px] flex items-center justify-center group">
              ...
              <div className="absolute top-4 left-4 p-4 rounded-xl bg-stone-charcoal/90 border border-sandstone/30 text-xs font-mono text-parchment backdrop-blur-md space-y-1.5 shadow-2xl z-20 max-w-sm">
                <div className="text-emerald-400 font-bold flex items-center gap-2">
                  ...
                  <span>YOLOv8 + BYTETRACK PIPELINE ACTIVE</span>
                </div>
                <div className="text-slate-300 font-bold">Video: {activeMetrics.name}</div>
                <div className="text-sandstone">
                  Detected People: <span className="text-emerald-400 font-bold">{activeMetrics.detected} PPL</span>
                </div>
                <div className="text-sandstone">
                ...
                <div className="text-temple-gold font-bold">
                  Avg Velocity: {activeMetrics.speed} | Density: {activeMetrics.density}
                </div>
              </div>
            </div>

            {/* REAL YOLOV8 MODEL METRICS CARDS */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-xs font-mono">
              <div className="p-4 rounded-xl bg-dusk-card border border-sandstone/20 space-y-1">
                <span className="text-sandstone/70 uppercase text-[10px]">YOLOV8 DETECTED PEOPLE</span>
                <div className="text-3xl font-bold text-emerald-400">{activeMetrics.detected} PPL</div>
              ...
              <div className="p-4 rounded-xl bg-dusk-card border border-sandstone/20 space-y-1">
                ...
                <div className="text-lg font-bold text-parchment">
                  <span className="text-emerald-400">{activeMetrics.moving} Moving</span> /{" "}
                  <span className="text-amber-400">{activeMetrics.stationary} Stat</span>
                </div>
                <span className="text-sandstone text-[10px]">Displacement Threshold: 5.0px</span>
              </div>

              <div className="p-4 rounded-xl bg-dusk-card border border-sandstone/20 space-y-1">
                <span className="text-sandstone/70 uppercase text-[10px]">AVERAGE FLOW VELOCITY</span>
                <div className="text-3xl font-bold text-temple-gold">{activeMetrics.speed}</div>
                <span className="text-sandstone text-[10px]">Calculated Speed Vectors</span>
              </div>
              ...
              <div className="p-4 rounded-xl bg-dusk-card border border-sandstone/20 space-y-1">
                ...
                <span className="text-slate-400 text-[10px]">Density Surge: {activeMetrics.density}</span>
              </div>
            </div>
          </section>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-xs font-mono">
            <div className="p-4 rounded-xl bg-dusk-card border border-sandstone/20 space-y-2">
              <div className="flex justify-between">
                ...
                <span className={`font-bold ${activeMetrics.status === "CRITICAL" ? "text-red-400" : "text-emerald-400"}`}>
                  {activeMetrics.density}
                </span>
              </div>
              <p className="text-[11px] text-sandstone/80">Wait time: 55 mins. YOLOv8 high surge flag active.</p>
            </div>
